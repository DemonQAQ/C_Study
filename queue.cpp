...
#define TYPE int
#define capacity 5//数组队列容量,实际容量+1 输入5则队列长为4
...
void menu_display(int flag)//菜单
{
	...
	printf("\t\tc.出队\n");
	printf("\t\tn.退出\n");
	printf("请输入字母选择功能(区分大小写,输入多个字符仅读取第一个):\n");
}
...
int scanf_overload(TYPE* value, long double min_edge, long double max_edge)//待输入的值，输入的上界和下界
{
	...
	do
	{
		...
		if (sizeof(TYPE) == 4) 
		{
			int x = 5;
			TYPE a = 2;
			if (x / a == 2)printf("请输入整数，范围为(%d - %d)\n", (int)min_edge, (int)max_edge);
			if (x / a == 2.5)printf("请输入数据，精度为小数点后7位，范围为(%f - %f)\n", (float)min_edge, (float)max_edge);
		}
		if (sizeof(TYPE) == sizeof(double))printf("请输入数据，精度为小数点后15位，范围为(%lf - %lf)\n", min_edge, max_edge);
